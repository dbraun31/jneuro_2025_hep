...
'from analysis.scripts.helpers.cluster_tools import (
...
permutation_cluster_test <- function(eeg,
    ...
    # Filter time
    eeg_s <- eeg[eeg$time >= time_window_min & eeg$time <= time_window_max,]
    
    
    # --- RUN STATISTICS --- #  
    if (median_s) {
        result <- py$permutation_cluster_test_med(eeg=eeg_s, initial_alpha=initial_alpha)
    } else {
        result <- py$permutation_cluster_test_num(eeg=eeg_s, initial_alpha=initial_alpha)
    }
...
plot_topo <- function(m, cluster_id, n_breaks=5, nrow=1, flip=FALSE) {
    ...
    if ('channel' %in% colnames(eeg)) {
        channels_total <- unique(eeg$channel)
    } else channels_total <- get_channels(eeg)
    ...
    t_obs <- result[['t_obs']]
    if (flip) t_obs <- t_obs * -1
    t_obs <- as.data.frame(t_obs[time_idxs, ])
    ...
    if (n_breaks > 1) {
        ...
        out <- plot_t_obs(t_obs, time_point = time_points)
    }
...
plot_t_obs <- function(t_obs, time_point) {
    ...
    p <- t_obs %>% 
        ...
        scale_fill_gradientn(limits = c(t_min, t_max), colors = colors,
                             breaks = c(-4, 3), labels = c('-4 (LA > HA)', '3 (HA > LA)')) + 
        theme_bw() + 
        theme(
