...
def permutation_cluster_test_num(eeg, initial_alpha=.01, time_window_min=0.25):
    '''
...
def permutation_cluster_test_med(eeg,
    ...
    ** REMOVE BADS BEFORE CALLING THIS FUNCTION **
    ...
    times_unique = sample_epochs.times
    time_window_idx = [i for i, e in enumerate(times_unique) if e >= time_window_min][0]
    times = times_unique[time_window_idx:]
...
def _format_med_for_clustering(eeg, sample_epochs):
    ...
    # Remove missing arousal values
    eeg = eeg.dropna(subset=['survey_n'])
